...
char bin2hex(uint8_t i) { assert((0 <= i) && (i < 16)); return (i<10)?('0'+i):('a'+i-10);}

uint8_t *hex_to_array(const unsigned char *array)
{
...
int main(int argc, char *argv[])
{
	...
	while (!quit) {
		...
		if (0) {
			...
			sscanf(buffer, "%*s %s\n", &path);
			ret = (long)fs_creat(path);
		} else if (!strcmp(command, "open")) {
			sscanf(buffer, "%*s %s\n", &path);
			ret = (long)fs_open(path);
		...
		/* Level 2 */
			...
			sscanf(buffer, "%*s %ld %s %d\n", &fd, &data, &len);
			ret = fs_write(fd, hex_to_array(data), len);
			...
			sscanf(buffer, "%*s %s\n", &path);
			ret = fs_stat(path, (fs_stat_t*)&buffer);
			if (ret>=0) {
				printf("%ld %s # %lu %lu\n", ret, array_to_hex(buffer, sizeof(fs_stat_t)), util_get_reads(), util_get_writes());
				continue;
			}
		/* Level 3 */
